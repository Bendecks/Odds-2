...
VERSION = os.getenv('RESEARCH_VERSION', 'phase2_1_gemini_research_v3')
...
def prompt(candidate):
    event = candidate.get('event_name')
    return (
        'You are a football research layer for a paper-only betting data pipeline. Use Google Search grounding.\n'
        f'Research this soccer/football match market: {event}, selection {candidate.get("selection")}, odds {candidate.get("odds")}, event UTC {candidate.get("event_time_utc")}.\n'
        'Find probability-changing facts only: injuries, suspensions, likely lineups, goalkeeper changes, fixture congestion, table motivation, weather/pitch if relevant, and market consensus if available.\n'
        'Prefer official club/league/competition sources and reputable sports media. Avoid betting-tip sites, affiliate pages, and odds-only previews. Do not treat odds movement as independent evidence.\n'
        'Return ONE valid JSON object only, no markdown. Keep arrays to max 3 items. Do not include long redirect URLs in source_links; use final public URLs if possible, otherwise [].\n'
        'Schema: {"research_status":"completed|insufficient_data|failed","source_links":[],"source_quality":{"primary_sources":[],"secondary_sources":[],"discarded_or_weak_sources":[],"echo_chamber_risk":"none|low|medium|high"},"signals":{"injuries":[],"lineups":[],"motivation":[],"form":[],"market_consensus":null,"contradictions":[]},"confidence":"low|medium|high","summary":"max 300 chars","research_flags":[]} '
    )
...
def fallback_from_text(candidate, text, chunks, parse_error):
    links = [x['uri'] for x in chunks if x.get('uri')][:3]
    ...
    return {
        ...
        'source_links': links,
        'source_quality': {'primary_sources': [], 'secondary_sources': links, 'discarded_or_weak_sources': ['raw_response_not_valid_json'], 'echo_chamber_risk': 'medium'},
        'signals': {'injuries': [], 'lineups': [], 'motivation': [], 'form': [], 'market_consensus': None, 'contradictions': []},
        'confidence': 'low',
        'summary': ('Gemini returned grounded text but not valid JSON. Raw response stored in error_detail for inspection.')[:300],
...
def gemini_research(candidate):
    ...
    body = {'contents': [{'role': 'user', 'parts': [{'text': prompt(candidate)}]}], 'tools': [{'google_search': {}}], 'generationConfig': {'temperature': 0, 'maxOutputTokens': 8192}}
    try:
        ...
        chunks = grounding_chunks(data)
        try:
            parsed = parse_json_text(text)
        except Exception as exc:
            parsed = fallback_from_text(candidate, text, chunks, exc)
        if chunks and not parsed.get('source_links'):
            parsed['source_links'] = [x['uri'] for x in chunks[:3]]
