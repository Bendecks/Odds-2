...
VERSION = os.getenv('RESEARCH_VERSION', 'phase2_1_gemini_research_v5')
...
def prompt(candidate):
    return f'''Use Google Search grounding. Return compact JSON only. No markdown. No URLs.
Task: research probability-changing football facts for paper-only model.
Match: {candidate.get('event_name')}
Selection: {candidate.get('selection')}
Odds: {candidate.get('odds')}
Event UTC: {candidate.get('event_time_utc')}
League: {candidate.get('league')}
Rules: prefer official/reputable sources; ignore betting-tip/affiliate/odds-only pages; do not count odds movement as evidence; keep each string under 120 chars.
Schema exactly:
{{"research_status":"completed|insufficient_data|failed","source_quality":{{"primary_source_count":0,"secondary_source_count":0,"echo_chamber_risk":"none|low|medium|high"}},"signals":{{"hard":[],"soft":[],"contradictions":[]}},"confidence":"low|medium|high","summary":"max 200 chars","research_flags":[]}}
'''
...
def fallback_record(text, chunks, parse_error):
    return {
        ...
        'source_quality': {'primary_source_count': 0, 'secondary_source_count': len(chunks), 'echo_chamber_risk': 'medium'},
...
def fake_output():
    return {'research_status': 'simulated', 'source_links': [], 'source_quality': {'primary_source_count': 0, 'secondary_source_count': 0, 'echo_chamber_risk': 'none'}, 'signals': {'hard': [], 'soft': [], 'contradictions': []}, 'confidence': 'low', 'summary': 'Simulated research record. Used only to verify plumbing.', 'research_flags': ['simulated_research_record', 'not_real_research'], 'grounding_chunks': []}
...
def normalize(candidate, output, error=None):
    if output is None:
        output = {'research_status': 'failed', 'source_links': [], 'source_quality': {'primary_source_count': 0, 'secondary_source_count': 0, 'echo_chamber_risk': 'high'}, 'signals': {'hard': [], 'soft': [], 'contradictions': []}, 'confidence': 'low', 'summary': f'Research failed or unavailable: {error_code(error)}', 'research_flags': [error_code(error)], 'grounding_chunks': []}
