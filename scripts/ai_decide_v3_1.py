...
MODEL_VERSION = os.getenv('AI_DECISION_VERSION', 'phase2_decision_v3_1_scoped_research_context')
...
def is_interesting_for_ai(candidate):
    ...
    if candidate.get('has_valid_research'):
        return True
    if movement == 'shortened' and change >= 0.03:
        return True
    return False
...
def compact_for_ai(candidate):
    keep = ['market_id', 'event_name', 'event_time_utc', 'market_type', 'line', 'selection', 'odds', 'movement_from_first', 'change_pct_from_first', 'has_valid_research', 'research_context']
    return {k: candidate.get(k) for k in keep}


def system_prompt():
    return 'Decision Layer V3.1. Paper-only. Use only provided JSON. Return JSON only. PAPER_BET requires valid research + market signal + no contradictions. Use PASS if unsure.'
...
def build_records(decisions, candidates, engine, fallback_reason=None, ai_eval_used=False):
    by_id = {c['market_id']: c for c in candidates}
    now = utc_now()
    records = []
    for d in decisions:
        ...
        records.append({'record_type': 'decision_record', 'decision_id': stable_hash('dec', MODEL_VERSION, d['market_id'], now), 'created_at': now, 'model_version': MODEL_VERSION, 'model': GEMINI_MODEL if engine == 'gemini' else 'heuristic_fallback', 'engine': engine, 'fallback_reason': fallback_reason, 'ai_eval_used': ai_eval_used and d['market_id'] in {x['market_id'] for x in candidates if x.get('_ai_eval')}, 'mode': 'paper_only', 'market_id': d['market_id'], 'event_id': c.get('event_id'), 'latest_observation_id': c.get('latest_observation_id'), 'event_name': c.get('event_name'), 'event_time_utc': c.get('event_time_utc'), 'market_type': c.get('market_type'), 'line': c.get('line'), 'selection': c.get('selection'), 'odds_at_decision': c.get('odds'), 'decision': d.get('decision'), 'confidence': d.get('confidence'), 'paper_stake_pct': d.get('paper_stake_pct'), 'reasoning_code': d.get('reasoning_code'), 'reasoning_summary': d.get('reasoning_summary'), 'risk_flags': d.get('risk_flags'), 'data_flags': d.get('data_flags'), 'research_used': rc is not None, 'research_id': rc.get('research_id') if rc else None, 'research_status': rc.get('research_status') if rc else None, 'research_confidence': rc.get('confidence') if rc else None, 'source_market_snapshot': c})
    return records
...
def main():
    ...
    records = build_records(decisions, candidates, engine, fallback_reason, ai_eval_used=bool(interesting and engine == 'gemini_scoped'))
    ...
    print(f'Decision Layer V3.1 OK | candidates={len(candidates)} ai_eval={len(interesting)} decisions={len(records)} engine={engine} fallback_reason={fallback_reason} report={path}')
