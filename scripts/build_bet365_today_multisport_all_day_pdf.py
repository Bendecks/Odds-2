...
def write_all_day_pdf(path, report_date, events, markets, args, excluded_count):
    ...
    story = [
        ...
        Paragraph('Separat all-day PDF. Det gamle multisport setup er ikke aendret.', styles['Normal']),
        Spacer(1, 10),
    ]
    ...
    for event in sorted_events:
        ...
        story.append(Paragraph(f"{event.get('date_denmark', '')} | {event.get('league', '')}", styles['Normal']))
...
def main():
    ...
    for sport_slug in args.sports:
        report_date, kept, excluded = base.fetch_events_for_sport(
            api_key,
            sport_slug,
            args.max_events,
            args.max_pages,
