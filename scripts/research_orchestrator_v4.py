...
VERSION = os.getenv('RESEARCH_VERSION', 'phase2_1_gemini_research_v4')
...
def prompt(candidate):
    return f'''You are a football research layer for a paper-only betting model.
...
Find only probability-changing facts: injuries, suspensions, likely lineups, goalkeeper changes, fixture congestion, table motivation, weather/pitch if relevant, and market consensus if available.
...
Do not treat odds movement as independent evidence.
Do NOT include URLs or links in the JSON. Sources are handled separately by the API grounding metadata.
Return exactly one JSON object inside a markdown code block labelled json.
Keep arrays to max 3 short strings.
...
def extract_grounding(data):
    ...
    # Keep report compact; store full chunk list in record, but source_links only first 3.
    return chunks


def fallback_record(text, chunks, parse_error):
    return {
        'research_status': 'completed_unstructured' if text and len(text.strip()) > 30 else 'insufficient_data',
        'source_quality': {'primary_source_count': 0, 'secondary_source_count': len(chunks), 'discarded_or_weak_sources': ['raw_response_not_valid_json'], 'echo_chamber_risk': 'medium'},
        'signals': {'injuries': [], 'lineups': [], 'motivation': [], 'form': [], 'market_consensus': None, 'contradictions': []},
        'confidence': 'low',
        'summary': 'Gemini returned grounded text but not valid JSON. Raw response stored for inspection.',
        'research_flags': ['json_parse_fallback', 'needs_prompt_tuning'],
        'grounding_chunks': chunks,
        '_error_detail': {'code': 'gemini_research_json_parse_fallback', 'parse_error': str(parse_error)[:500], 'response_text': str(text)[:4000]},
    }
...
def fake_output():
    return {'research_status': 'simulated', 'source_links': [], 'source_quality': {'primary_source_count': 0, 'secondary_source_count': 0, 'discarded_or_weak_sources': [], 'echo_chamber_risk': 'none'}, 'signals': {'injuries': [], 'lineups': [], 'motivation': [], 'form': [], 'market_consensus': None, 'contradictions': []}, 'confidence': 'low', 'summary': 'Simulated research record. Used only to verify plumbing.', 'research_flags': ['simulated_research_record', 'not_real_research'], 'grounding_chunks': []}
...
def normalize(candidate, output, error=None):
    if output is None:
        output = {'research_status': 'failed', 'source_links': [], 'source_quality': {'primary_source_count': 0, 'secondary_source_count': 0, 'discarded_or_weak_sources': [], 'echo_chamber_risk': 'high'}, 'signals': {'injuries': [], 'lineups': [], 'motivation': [], 'form': [], 'market_consensus': None, 'contradictions': []}, 'confidence': 'low', 'summary': f'Research failed or unavailable: {error_code(error)}', 'research_flags': [error_code(error)], 'grounding_chunks': []}
...
def write_report(payload, records):
    ...
    for r in records:
        ...
        lines += ['', f'### {r.get("event_name")} — {r.get("selection")} @ {r.get("odds")}', f'- Status: {r.get("research_status")}', f'- Provider: {r.get("provider")}', f'- Confidence: {r.get("confidence")}', f'- Echo chamber risk: {sq.get("echo_chamber_risk")}', f'- Source counts: primary={sq.get("primary_source_count")}, secondary={sq.get("secondary_source_count")}', f'- Triggers: `{r.get("trigger_reasons")}`', f'- Summary: {r.get("summary")}', f'- Source links: `{r.get("source_links")}`', f'- Research flags: `{r.get("research_flags")}`', f'- Error code: `{err.get("code") if isinstance(err, dict) else None}`', f'- Error detail: `{err.get("response_text") if isinstance(err, dict) else None}`']
