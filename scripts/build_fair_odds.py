...
league_home_avg = ((matches['FTHG'] * matches['season_weight']).sum() / matches['season_weight'].sum())
league_away_avg = ((matches['FTAG'] * matches['season_weight']).sum() / matches['season_weight'].sum())
...
for team in teams:
    ...
    home_attack = weighted_avg(home_matches['FTHG'], home_matches['season_weight'], league_home_avg)
    home_defense = weighted_avg(home_matches['FTAG'], home_matches['season_weight'], league_away_avg)
    away_attack = weighted_avg(away_matches['FTAG'], away_matches['season_weight'], league_away_avg)
    away_defense = weighted_avg(away_matches['FTHG'], away_matches['season_weight'], league_home_avg)

    team_recent = pd.concat([
        pd.DataFrame({'goals_for': home_matches['FTHG'], 'goals_against': home_matches['FTAG'], 'weight': home_matches['season_weight']}),
        pd.DataFrame({'goals_for': away_matches['FTAG'], 'goals_against': away_matches['FTHG'], 'weight': away_matches['season_weight']}),
    ]).tail(8)

    recent_attack = weighted_avg(team_recent['goals_for'], team_recent['weight'], league_goal_avg) / league_goal_avg
    recent_defense = weighted_avg(team_recent['goals_against'], team_recent['weight'], league_goal_avg) / league_goal_avg
...
ratings['attack_strength'] = ratings['home_attack_strength'] * 0.35 + ratings['away_attack_strength'] * 0.35 + ratings['recent_attack_strength'] * 0.30
ratings['defense_strength'] = ratings['home_defense_strength'] * 0.35 + ratings['away_defense_strength'] * 0.35 + ratings['recent_defense_strength'] * 0.30
...
forward_prediction_script = Path('scripts/build_forward_fixture_predictions.py')
if forward_prediction_script.exists():
    try:
        runpy.run_path(str(forward_prediction_script), run_name='__main__')
    except Exception as exc:
        print(f'Forward fixture prediction build skipped: {exc!r}')
